...
class ArticlesRepository(BaseRepository):  # noqa: WPS214
    ...
    async def create_article(  # noqa: WPS211
        ...
        *, 
        ...
        # 失效文章列表缓存
        await invalidate_cache(["articles:list:*"])
    ...
    async def update_article(  # noqa: WPS211
        ...
        *, 
        ...
        # 失效文章详情和列表缓存
        await invalidate_cache([f"article:detail:{article.slug}:*", "articles:list:*"])
        if updated_article.slug != article.slug:
            await invalidate_cache([f"article:detail:{updated_article.slug}:*"])
    ...
    async def delete_article(self, *, article: Article) -> None:
        ...
        await invalidate_cache([
            f"article:detail:{article.slug}:*", 
            "articles:list:*", 
            f"comments:article:{article.slug}:*"
        ])

    @cache(
        key_pattern=CacheKeys.articles_list(page="{offset//limit + 1}", limit="{limit}", tag="{tag}", author="{author}", favorited="{favorited}"),
        ttl=300,  # 5分钟
        invalidate_patterns=["articles:list:*"]
    )
